...
cradle.Connection.prototype.rawRequest = function (options, callback) {
    ...
    function instrumentedCallback(err, res, body) {
      var end = Date.now();
      console.log('cradle:performance: %s', JSON.stringify({
        latency: end - start,
        options: {
          method: options.method,
          query: options.query,
          body: options.body && options.body.substr(0, 150),
          uri: options.uri
        },
        err: err,
    ...
    return request(options, instrumentedCallback);
};
